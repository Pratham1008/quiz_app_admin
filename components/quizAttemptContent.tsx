...
export default function QuizAttemptContent({ quizId }: { quizId: string }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const questionRes = await authFetch(
                    `https://api.prathameshcorporation.info/student/questions/${quizId}`
                );
                const questionText = await questionRes.text();
                if (!questionRes.ok) throw new Error(`HTTP ${questionRes.status}`);
            ...
            try {
                const res = await authFetch(
                    `https://api.prathameshcorporation.info/student/quiz/remaining-time/${sessionId}`
                );
                const data: { startTime: string; duration: number } = await res.json();
                ...
                if (err instanceof Error) {
                    alert(`Failed to fetch quiz timer , ${err.message}`);
                }
            } finally {
                setLoading(false);
            }
    ...
    const handleSubmit = useCallback(() => {
        ...
        const sessionAnswers = questions.map(
            (q) => answers[q.questionId] || "not answered"
        );
        ...
        authFetch(
            `https://api.prathameshcorporation.info/student/quiz/submit/${sessionId}?uid=${auth.currentUser?.uid}`,
            {
                method: "POST",
            ...
            <AnimatePresence>
                {loading ? (
                    ...
                    >
                        ...
                        <Skeleton className="h-6 w-full rounded-lg" />
                        <Skeleton className="h-6 w-full rounded-lg" />
                        <Skeleton className="h-10 w-1/2 mt-8 rounded-xl" />
                    </motion.div>
                ) : (
                    <motion.main
                        key="quiz"
                        ...
                        exit={{ opacity: 0 }}
                        className="bg-white rounded-2xl shadow-md p-8 flex flex-col justify-between h-[480px]"
                    >
                        ...
                        <nav className="flex justify-between">
                            ...
                            {currentIndex < questions.length - 1 ? (
                                ...
                                <Button
                                    ...
                                    className="bg-green-600 hover:bg-green-700 text-white shadow-sm flex gap-2 items-center"
